...
def _run():
    ...
    if not args.variables_file:
        cfg = Config.get_cfg(int(args.category),args.runtag,False)
        brs = cfg.brs
        if cfg.scale:
          #with open('./data/scale_{0:}_{1:}.pickle'.format(cfg.runtag,cfg.train_tag),'rb') as fscale:
          #  scaler = pickle.load(fscale)
          with open('./data/scale_{0:}_{1:}.json'.format(cfg.runtag,cfg.train_tag),'r') as fscale:
            scaler = json.load(fscale)
        else:
          scaler = None
        _build_variables_file(args,brs,scaler)
        sys.exit(0)
...
def _build_variables_file(args,brs=None,scaler=None):
    with open(args.arch_file, 'r') as arch_file:
        arch = json.load(arch_file)
    ...
    input_layer_arch = arch['config']['input_layers']
    vars_per_input = _get_vars_per_input(input_layer_arch, node_dict)
    def get_input(n,brs=brs,scaler=scaler):
        if brs!=None:
          var_name = brs[n]
        else:
          var_name = 'variable_{}'.format(n)

        if scaler != None:
          scale_,offset_ = scaler[brs[n]]['scale'],scaler[brs[n]]['offset']
        else:
          scale_,offset_ = 1,0
        return {'name': var_name, 'scale': scale_, 'offset':offset_}

    def build_inputs(input_items,brs=brs,scaler=scaler):
        inputs = []
        for nodenum, n_vars in input_items:
            the_input = {
                'name': 'node_{}'.format(nodenum),
                'variables': [get_input(n,brs,scaler) for n in range(n_vars)]
            }
